#!/bin/bash
# Custom bootstrap script for Empire C2 on Digital Ocean droplet 
...
sed -i 's/if \[ "\$EUID" -eq 0 \]; then/if \[ "\$EUID" -eq 0 \]; then #/' /root/Empire/setup/install.sh
sed -i 's/    exit 1/    #exit 1/' /root/Empire/setup/install.sh
